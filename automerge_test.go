...
func TestMap(t *testing.T) {
	...
	// create new map
	m := automerge.NewMap()
	require.NoError(t, doc.RootMap().Set("x", m))
	...
	n := automerge.NewMap()
	require.NoError(t, doc.RootMap().Set("n", n))
	n.Set("string", "hello")
...
func TestList_Path(t *testing.T) {
	...
	require.EqualError(t, err, "&automerge.Path{\"y\", 0}: tried to read non-list \"b\"")
...
func TestChangeHash(t *testing.T) {
	h, err := automerge.NewChangeHash("c72e8c6e659a7aaa46d4066dcded289368f48c8868d6d724f5c79ff51949796c")
	require.NoError(t, err)
	require.Equal(t, h.String(), "c72e8c6e659a7aaa46d4066dcded289368f48c8868d6d724f5c79ff51949796c")

	_, err = automerge.NewChangeHash("ab")
	require.Equal(t, err.Error(), "automerge.NewChangeHash: expected 32 bytes, got 1")

	_, err = automerge.NewChangeHash("yz")
	require.Error(t, err)
}
